var EventTree = function(name) {
	...
	$().w2sidebar({ 
		...
		onClick : function(e) {
			if (e.object.eventNode.type == 'TRADING') {
				console.log('event reached, populate form');
				$.trigger(this, $this.componentName + '_eventSelected', e.object.eventNode);
			}
...
EventTree.prototype = {
	...
	onGetSportsRootNodes: function(getSportsRootNodesModel)
	{
		...
		for (var i in resultNodes) {
			var n = resultNodes[i];
			nodes.push({id: 'n-' + n.id, text: n.name, img: 'icon-folder', plus: true, nodes : [], eventNode : n});
		}
	...
	onGetSportsNode: function(getSportsNode) 
	{
		...
		if (node.hasChildren) {
			...
			for (var i in node.children) {
				...
				var child = {id: 'n-' + n.id, text: n.name, eventNode : n};
